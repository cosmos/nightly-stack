...
COSMOS_MONIKER=${COSMOS_MONIKER:-node}
COSMOS_NODE_HOME=${COSMOS_NODE_HOME:-/config}
COSMOS_START_CMD=${COSMOS_START_CMD:-start}

if [[ ! -f "${COSMOS_NODE_HOME}/config.json" ]]; then
