...
if [[ ! -f "${HOME}/config/config.toml" ]]; then
    ...
    if [ "$GENESIS_VALIDATORS_NUMBER" -gt 1 ]; then
        "${COSMOS_NODE_CMD}" keys show "${validator}" --home "${HOME}" > "${SHARED_VOLUME}/${validator}.keys"
    fi
        ...
        if [ "$GENESIS_VALIDATORS_NUMBER" -gt 1 ]; then
            ...
            for ((i=1; i<=$GENESIS_VALIDATORS_NUMBER; i++)); do
                if [ "${validator}" != "validator-${i}" ]; then
                    KEY_FILE="${SHARED_VOLUME}/validator-${i}.keys"
                    FOUND=false
                    ...
                    while [ "$FOUND" = false ]; do
                        ...
                        if [ $ELAPSED_TIME -ge $TIMEOUT ]; then
                            echo "Timeout reached after $TIMEOUT seconds waiting for ${KEY_FILE}"
                            exit 1
                        fi

                        if [ -f "$KEY_FILE" ]; then
                            echo "Found key file: ${KEY_FILE}"
                            echo $(cat "${SHARED_VOLUME}/validator-${i}.mnemonic") | "${COSMOS_NODE_CMD}" keys add "validator-${i}" --recover --home "${HOME}" || true
                            FOUND=true
                        else
                            echo "Waiting for ${KEY_FILE}: $(date '+%H:%M:%S') (${ELAPSED_TIME}s elapsed)"
                            sleep 3
                        fi
                    done
                fi
